...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /*======== 4. JVECTORMAP USA REGIONS VECTOR MAP ========*/
  const usVectorMap = document.getElementById("us-vector-map-marker");
  if (usVectorMap) {
      $(usVectorMap).vectorMap({
          map: "us_aea",
          backgroundColor: "transparent",
          ...
          markers: [
              ...
              { latLng: [43.8, -120.55], name: "Oregon", style: { fill: "#9e6de0", stroke: "#9e6de0" }},
          ],
      });
  }

  /*======== 5. COUNTRY SALES RANGS ========*/
  const countrySalesRange = document.getElementById("country-sales-range");
  if (countrySalesRange) {
      const start = moment().subtract(29, "days");
      const end = moment();

      function cb(start, end) {
          document.querySelector("#country-sales-range .date-holder").innerHTML =
              start.format("MMMM D, YYYY") + " - " + end.format("MMMM D, YYYY");
      }
  ...
  const miniStatusRanges = document.getElementById("mini-status-range");
  if (miniStatusRanges) {
      const start = moment().subtract(29, "days");
      const end = moment();

      function cb(start, end) {
          document.querySelector("#mini-status-range .date-holder").innerHTML =
              start.format("MMMM D, YYYY") + " - " + end.format("MMMM D, YYYY");
      }
  ...
  /*======== 6. JVECTORMAP HOME WORLD ========*/
  const countryWithMarker = document.getElementById("world-country-with-marker");
  if (countryWithMarker) {
      const colorData = {
          CA: 106,
          US: 166,
          RU: 166,
  ...
  const usVectorMapWithoutMarker = document.getElementById("us-vector-map-without-marker");
  if (usVectorMapWithoutMarker) {
      $(usVectorMapWithoutMarker).vectorMap({
          map: "us_aea",
          backgroundColor: "transparent",
          zoomOnScroll: false,
  ...
  if (codeEditor) {
      const htmlCode = `<html style="color: green">
  <!-- this is a comment -->
  <head>
    <title>HTML Example</title>
  </head>
  <body>
    ...
    I mean&quot;</em>... but might not match your style.
      ...
      const myCodeMirror = CodeMirror(codeEditor, {
          value: htmlCode,
          mode: "xml",
          extraKeys: { "Ctrl-Space": "autocomplete" },
          lineNumbers: true,
  ...
  /*======== 8. QUILL TEXT EDITOR ========*/
  const quillHook = document.getElementById("editor");
  if (quillHook) {
      const quill = new Quill(quillHook, {
          modules: {
              formula: false,
              syntax: false,
              toolbar: "#toolbar",
  ...
  /*======== 9. MULTIPLE SELECT ========*/
  const select2Multiple = document.querySelectorAll(".js-example-basic-multiple");
  select2Multiple.forEach((element) => {
      $(element).select2();
  });

  const select2Country = document.querySelectorAll(".country");
  select2Country.forEach((element) => {
      $(element).select2({
          minimumResultsForSearch: -1,
      });
  });
  ...
  /*======== 10. LOADING BUTTON ========*/
  const laddaButton = document.querySelectorAll(".ladda-button");
  laddaButton.forEach((element) => {
      Ladda.bind(element, { timeout: 1000 });
  });
  ...
  carousel.forEach((element) => {
      const bsCarousel = new bootstrap.Carousel(element, {
          interval: 2000,
          wrap: true,
      });
  });
  ...
  progressBars.forEach((element) => {
      const progressValue = element.getAttribute("aria-valuenow");
